...
$(document).ready(function () {
	...
	function setupMobileMenu() {
		...
		require(['pulling', 'storage'], function (Pulling, Storage) {
			...
			function setupSetting() {
				if (ajaxify.data.template['account/settings'] && !document.getElementById('ariastel:menus:legacy-layout')) {
					require(['translator'], function (translator) {
						translator.translate('[[ariastel:mobile-menu-side]]', function (translated) {
							$('<div class="well checkbox"><label><input type="checkbox" id="ariastel:menus:legacy-layout"/><strong>' + translated + '</strong></label></div>')
								.appendTo('#content .account > .row > div:first-child')
								.find('input')
								.prop('checked', Storage.getItem('ariastel:menus:legacy-layout', 'true'))
								.change(function (e) {
									if (e.target.checked) {
										Storage.setItem('ariastel:menus:legacy-layout', 'true');
									} else {
										Storage.removeItem('ariastel:menus:legacy-layout');
									}
								});
						});
					})
	...
	function setupCardRemoval(card) {
		function removeCard(ev) {
			if ($(ev.target).closest('.ariastel-usercard').length === 0) {
		...
		$(window).on('action:ajaxify.end', function (ev, data) {
			if (data.url && data.url.match('^topic/')) {
				if (config.enableQuickReply) {
					require(['ariastel/quickreply'], function (quickreply) {
						if (quickreply) {
							quickreply.init();
						}
					});
